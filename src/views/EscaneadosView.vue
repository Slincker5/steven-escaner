...
<template>
  <div class="p-4 overflow-y-auto">
    <div
      class="py-4 font-medium flex items-center justify-between"
      v-if="noEscaneados"
    >
    ...
    <div
      v-if="escaneados && escaneados.length > 0"
      v-for="articulo in escaneados"
      class="p-4 border border-solid border-[#ddd] mb-4"
    >
      <div class="truncate font-medium">
        <font-awesome-icon :icon="['fas', 'tag']" /> {{ articulo.descripcion }}
      </div>
      <div class="py-2 text-sm">SKU: {{ articulo.articulo }}</div>
      <div class="py-2 text-sm">
        Escaneado: {{ dayjs(articulo.fecha).fromNow() }}
      </div>
