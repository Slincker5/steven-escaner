...
const { categoriaSeleccionada, listadoDeCategorias, categoriaErrorSeleccion } =
  storeToRefs(categoria);
...
<template>
  <div>
    <div class="relative inline-block" v-if="mensaje.categoriaExistencia">
      <button
        @click.prevent="toggleOpciones"
        class="border border-solid px-3 py-2 text-sm rounded-md font-medium shadow-sm shadow-black/40 uppercase textoCategoria transition-colors"
        :class="
          categoriaErrorSeleccion === true
            ? 'border-[#797979] text-gray-900'
      ...
      <nav
        ...
        :class="[
          'absolute left-0 mt-2 w-48 h-44 overflow-y-scroll bg-white border border-gray-300 rounded-md shadow-lg z-30 uppercase noscroll',
          mostrarOpciones ? 'bounce-in' : '',
        ]"
      >
        <ul v-for="item in listadoDeCategorias">
          <li
            @click.prevent="seleccionarOpcion(item.titulo, item.uuid)"
            class="text-ellipsis text-nowrap overflow-hidden px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm"
          >
            <i class="fa-jelly fa-regular fa-tag"></i> {{ item.titulo }}
          </li>
        </ul>
      </nav>
    </div>
